...
public static class WindowFrameTheme
{
    ...
    public static void AttachAll()
    {
        ...
        {
            // A window that is only now being drawn picks the attribute up on its own — no repaint needed.
            Apply(window, repaint: false);
    ...
    /// <summary>
    /// Forces the title bar of an already-visible window to redraw. Switching the theme at runtime
    /// otherwise leaves the old bar standing until the window loses and regains focus, because Windows 10
    /// only repaints the non-client area on an activation change — so replay exactly that.
    /// </summary>
